...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function fetchAlbumImages(albumName) {
    ...
    const images = Array.from({ length: count }, (_, i) => 
        // `../assets/gallery/album/${folderName}/${prefix.toLowerCase()}-${i + 1}.jpg` // for photo at local
        `https://ik.imagekit.io/zzzhehao/gallery/album/${folderName}/${folderName}-${i + 1}.jpg` // for photo on imagekit
    );
  ...
  function getFullImageUrl(src) {
    ...
    return `https://ik.imagekit.io${path}`;
    // return src;
  }
