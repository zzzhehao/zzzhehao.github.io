...
files <- list.files(path = 'assets/gallery/album', recursive = T, full.names = T)

set.seed(as.numeric(today()))
pick <- round(runif(n = 1, max = length(files)), digit = 0)
file <- files[pick]
file.name <- basename(file) %>% sub("\\.jpg$", "", .)
url <- paste0('https://zzzhehao.github.io/', file)

cat(
    "::: {.name-title style=\"font-size: 1.5rem\"}
Today's Pick", "\n", ":::", "\n",

    "::: {.image-container}", "\n",
    "::: {.pick-image}", "\n",
    "[![](", url, ')](', "https://zzzhehao.github.io/gallery/gallery.html#", file.name, ')', "\n", 
    ":::", "\n", ":::",
    sep = "")
```
