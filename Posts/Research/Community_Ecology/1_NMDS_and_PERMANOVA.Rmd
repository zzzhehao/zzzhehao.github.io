---
title: Community Ecology 1 - NMDS and PERMANOVA
date: "2024-06-29"
draft: false
---
...
```{r setup, echo = F, results=F, warning=F, error=F, message=F}
library(dplyr)
library(vegan)
...
```{r data_import, echo=T, message=F, warning=F}
...
```{r showing_depth_data, fig.height=6, fig.width=12, warning=F, message=F}
...
```{r aes_setting, echo=F, warning=F, message=F}
theme_aes <- function(clp = "Klein", base_family = "serif", ...){
...
```{r extract_points_and_plot, warning=F, error=F, fig.width=5, fig.height=5}
# extract NMDS scores
...
```{r spider_plot, warning=F, message=F}
cat <- "Speed_cat"
...
```{r hullplot, warning=F, message=F}
...
```{r ggfitplot, warning=F, message=F, fig.width=10, fig.height=11}
...
```{r geom_contour_filled, warning=F, message=F}
p.NMDS.sp.st.depth <- ggplot(data = spCoord, aes(x = NMDS1, y = NMDS2))+
  ...
  geom_point(data = siteCoord, aes(x = NMDS1, y = NMDS2), color = "#222222", size = 3.5, alpha = 0.5) +
  geom_text(data = siteCoord, aes(x = NMDS1, y = NMDS2, label = c(1:nrow(siteCoord))), color = "white", size = 1.8, alpha = 1, check_overlap = T) +
  scale_fill_manual(values = moma.colors("Ernst", 20)) +
  coord_equal() +
  theme_aes() + # available in source code Rmd
...
adonis2(tadpoleSp ~ env.comp$`stream slope [cm/30m]`)
```
